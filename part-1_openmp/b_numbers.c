...
int main(int argc, char *argv[]) {
    ...
    for (int round = 0; round < R; ++round) {
        ...
        for (int hit = 0; hit < hit_amount; ++hit) {
            ...
            int row_start_offset = row_start - (row_start % omp_get_max_threads());
            ...
            #pragma omp parallel for schedule(static, 1) proc_bind(close)
            for (int row = row_start_offset; row < (row_end > rows ? rows : row_end); row++) {
    ...
    if (env_hostname == NULL || strncmp(env_hostname, "its-", 4) != 0) {
        ...
        // display sample coordinates from stdin
        printf("a[%d][%d]=%d\n", 80, 15, matrix[80][15]);
    }
